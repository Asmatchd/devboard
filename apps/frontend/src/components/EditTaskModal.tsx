...
export function EditTaskModal({ task, onClose }: EditTaskModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const updated = await taskService.update(task.id, { title, description, status });
      ...
      <form onSubmit={handleSubmit} className="flex flex-col" style={{ gap: "24px" }}>
        ...
        <div className="flex flex-col" style={{ gap: "8px" }}>
          <div className="flex items-center justify-between">
            <label className="font-medium text-(--color-text-secondary)" style={{ fontSize: "14px" }}>
              Description
            </label>
            <button
              type="button"
        ...
        <div className="flex flex-col" style={{ gap: "8px" }}>
          ...
          <div className="relative">
            ...
            <div
              ...
              style={{ right: "14px", top: "50%", transform: "translateY(-50%)", fontSize: "12px" }}
            >
              ▼
            </div>
          </div>
